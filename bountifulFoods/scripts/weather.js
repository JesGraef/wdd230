...
async function apiFetchWeather() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Weather',data); // this is for testing the call
  ...
  async function apiFetchForecast() {
    const url = 'https://api.openweathermap.org/data/2.5/forecast?q=Carlsbad,US&units=imperial&appid=d5ff9422b0221d7c0207bd4aa38a8886';
    try {
      const response = await fetch(url);
      if (response.ok) {
        const data = await response.json();
